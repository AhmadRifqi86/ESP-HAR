...
export default function Dashboard() {
  ...
  async function GetMeasure() {
    setIsLoading(true);
    const response = await fetch("http://localhost:8000/getM", {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // Adjust the content type if needed,
        Authorization: `Bearer ${token}`,
    ...
    const response2 = await fetch("http://localhost:8000/user/health-status", {
      method: "POST",
      headers: {
        "Content-Type": "application/json", // Adjust the content type if needed,
        Authorization: `Bearer ${token}`,
          ...
          <div className="space-y-4">
            <div className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 sm:grid-cols-1">
                    ...
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      ...
                      <div className="flex flex-col items-center pl-4">
                        <p className=" font-semibold text-lg">
                          {user?.health_status?.heart_rate_status ===
                            "Moderate" && (
                            <span className="text-green-800">
                              {
                                user?.measure[user?.measure?.length - 1]
                                  ?.heart_rate_data
                              }{" "}
                              BPM
                            </span>
                    ...
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <div className="flex items-center mt-2 justify-center">
                        <h2 className="font-semibold">
                          ...
                          {user?.emergencyContactEmail || "ieee@supcom.tn"}{" "}
                        </h2>
                      </div>
                    </div>
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      ...
                      <div className="flex flex-col items-center pl-4">
                        ...
                        {user?.health_data.gender === 2 && (
                          <p className=" font-semibold">Men</p>
                        )}
                        {user?.health_data.gender === 1 && (
                  ...
                  <CardContent className="m-4 grid items-center">
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      ...
                      <div className="flex flex-col items-center pl-4">
                        <p className=" font-semibold text-lg">
                          <span className="text-green-800">
                            {user?.measure[user?.measure?.length - 1]?.temp}°C
                          </span>{" "}
                        </p>
                      </div>
                    </div>
                ...
                <Card>
                  ...
                  <CardContent className="m-4 grid items-center">
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      ...
                      {/* Column 2 */}
                      <div className="flex flex-col items-center pl-4">
                        {user?.health_status?.bmi_status === "Normal" && (
                          <h2 className=" font-bold text-green-800">
                            {parseFloat(user?.health_status?.bmi).toFixed(2)}
                          </h2>
                        )}
                        {user?.health_status?.bmi_status !== "Normal" && (
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>systolic and diastolic pressure</CardTitle>
                  </CardHeader>
                  <CardContent className="m-4 grid items-center">
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      {/* Column 1 */}
                      ...
                      {/* Column 2 */}
                      <div className="flex flex-col items-center pl-4">
                        {user?.health_status?.blood_pressure_status ===
                          "Normal" && (
                          <h2 className=" font-bold text-green-800">
                            <span className=" text-gray-500">sys:</span>
                            {user?.measure[user?.measure?.length - 1]?.ap_hi}
                          </h2>
                        )}
                        {user?.health_status?.blood_pressure_status !==
                          "Normal" && (
                        ...
                        {user?.health_status?.blood_pressure_status !==
                          ...
                          <h2 className=" font-bold text-red-800">
                            ...
                            {user?.measure[user?.measure?.length - 1]?.ap_lo}
                          </h2>
                        )}
                        {user?.health_status?.blood_pressure_status ===
                          "Normal" && (
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>Oxygyne level</CardTitle>
                  </CardHeader>
                  <CardContent className="m-4 grid items-center">
                    <div className=" mt-3 grid grid-cols-2 gap-4 items-center justify-center">
                      {/* Column 1 */}
                      ...
                      {/* Column 2 */}
                      <div className="flex flex-col items-center pl-4">
                        {user?.health_status?.saturation_status ===
                          "Normal" && (
                          <h2 className=" font-bold text-green-800">
                            {
                              user?.measure[user?.measure?.length - 1]
                  ...
                  <CardFooter className="flex items-center justify-center">
                    <div className=" mt-10 flex items-center justify-center">
                      {user?.health_status?.saturation_status === "Normal" && (
                        <h2 className=" font-bold text-green-800">
                          {user?.health_status?.saturation_status}
                        </h2>
                      )}
